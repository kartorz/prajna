...
public class TCLogParser implements ILogParser{
    protected static final Logger log = LogManager.getLogger(TCLogParser.class.getName());
     ...
     */
    public int parse(int parseCount) {
        if (this.SOURCE == null) {
        ...
        try {
            ...
            return parse(this.READER, parseCount);
     ...
     * @param breader
     */
    protected int parse(BufferedReader breader, int parseCount) {
        int actualCount = 0;
        String line = null;
        try {
            ...
            while (line != null) {
            	...
            	this.RIP = null;
                if (line.length() > 0) {
                    actualCount += this.parseLine(line);
                }
    ...
    protected int parseLine(String line) {
        ...
        if (FILTER != null) {
            ...
            if (!FILTER. isFiltered(line)) {
                ...
                if (line != null) {
                	notifyParseUrl(RIP, RMETHOD, cleanedLine);
                }
            } else {
    ...
    public boolean checkIP(String entry) {
        int pos = entry.indexOf(" ");
        if (pos > -1) {
            this.RIP = entry.substring(0, pos);
            return true;
        } else {
            return false;
        }
    ...
    private void notifyParseUrl(String IP, String method, String url){
        for(ParserListener listener : listeners) {
            listener.onParseUrl(IP, method, url);
        }
    }
    
    private void notifyParseDone(){
    	for(ParserListener listener : listeners) {
             listener.onParseDone();
        }
    } 
}